...
public:
	...
	void Push(SDL_Surface *surface, Transform &transform, double angle) {
		ASSERT(surface != nullptr);
		auto texture = SDL_CreateTextureFromSurface(m_SDLRenderer, surface);
		transform.x -= surface->w / 2;
		transform.y -= surface->h / 2;
		...
		SDL_RenderCopyEx(m_SDLRenderer, texture, nullptr, &transform(), angle,center , flip);
		SDL_FreeSurface(surface);
		SDL_DestroyTexture(texture);
	}
	void Push(const ObjectID &objectID, const Transform &transform, double angle) {
		SDL_Point* center = NULL;
		SDL_RendererFlip flip = SDL_FLIP_NONE;
